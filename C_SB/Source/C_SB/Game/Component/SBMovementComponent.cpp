...
void USBMovementComponent::AddInitMoveRecord(const Protocol::PosInfo& PosInfo, const Protocol::MoveState& State)
{
	ISBObject* OwnSBObject = GetOwner<ISBObject>();
	check(OwnSBObject);

	if (OwnSBObject->IsMyPlayer() == false)
	{
		Protocol::MoveState InitState = State;

		if (InitState == Protocol::MOVE_STATE_IDLE || InitState == Protocol::MOVE_STATE_NONE)
			return;

		if (InitState == Protocol::MOVE_STATE_FOLLOW_PATH)
			InitState = Protocol::MOVE_STATE_WALK;
		
		AddRecord(PosInfo, Protocol::DestInfo(), InitState);
	}
}
...
void USBMovementComponent::NotifyPathFollowing(FVector DestLocation)
{
	...
	// 이전 전송 움직임 방향과 차이가 있어 빠른 전송이 필요한가
	FVector PathFollowingDir = FVector(Dest.x(), Dest.y(), 0.) - PosUtils::Pos2Vector(Pos);
...
void USBMovementComponent::ReplicateToServer(const uint64 CurrentTime, float DeltaTime)
{
	...
	if (_MovePacketSendTimer <= SendTimerThreadHold)
	{
		...
		if (NextRecord != nullptr)
		{
			...
			{
				FVector NewMoveDir = FVector(NextRecord->dest_info().x(), NextRecord->dest_info().y(), 0.) - PosUtils::Pos2Vector(NextRecord->pos_info());
			...
			{
				_MoveDir = FRotationMatrix(PosUtils::Pos2Rotator(NextRecord->pos_info())).GetUnitAxis(EAxis::X);
			}
	...
	while (_Records.IsEmpty() == false)
	{
		...
		*_State = NextState;
		...
		else if (*_State != Protocol::MOVE_STATE_IDLE && *_State != Protocol::MOVE_STATE_NONE)
		{
			...
			{
				CalcRevisedMove(CurrentTime, PosUtils::Pos2Vector(*OwnSBObject->GetPosInfo()), PosUtils::Pos2Vector(NextPosInfo));
			}
...
{
	check(PredictedVelocityDelegate.IsBound());
	float Velocity = PredictedVelocityDelegate.Execute(*_State);

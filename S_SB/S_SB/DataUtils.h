...
{
	TacticsConvertor(uint32 inBytes)
	{
		bytes = inBytes;
	}
...
{
	BreakCountsConvertor(uint32 inBytes)
	{
		bytes = inBytes;
	}
...
{
	CostumeSettingConvertor(uint32 inBytes)
	{
		bytes = inBytes;
	}
...
{
	ScoreBoardConvertor(uint64 inBytes)
	{
		bytes = inBytes;
	}
...
{
	PhysicalInfoConvertor(uint64 inBytes)
	{
		bytes = inBytes;
	}
...
{
	MasteryConvertor(uint64 inBytes)
	{
		bytes = inBytes;
	}
...
{
	InjuryInfoConvertor(uint32 inBytes)
	{
		bytes = inBytes;
	}
...
public:
	...
	static AthleteInstanceRef CreatedAthleteInstance(const xWString name);
};

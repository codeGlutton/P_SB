...
public:
	...
	// 연속으로 위치한 값들 BindParam 특수화 using BYTE
	template<typename T>
	...
	// 배열 BindCol 특수화 using BYTE
	template<typename T, int32 N>
	void BindCol(int32 idx, T(&value)[N])
	{
		_dbConnection.BindCol(idx + 1, (BYTE*)value, sizeof(T) * N, &_columnIndex[idx]);
		_columnFlag |= (1LL << idx);
	}
